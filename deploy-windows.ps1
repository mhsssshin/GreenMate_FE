# GreenMate Frontend 배포 스크립트 (Windows PowerShell)
# 사용법: .\deploy-windows.ps1
...
"@
    ...
    # expect 실행 (Windows에서는 WSL이나 Git Bash 필요)
    if (Get-Command wsl -ErrorAction SilentlyContinue) {
        wsl expect temp_ssh.exp
    } elseif (Get-Command bash -ErrorAction SilentlyContinue) {
        bash -c "expect temp_ssh.exp"
    } else {
        Write-Error "expect 또는 WSL이 필요합니다. 다음 중 하나를 설치하세요:"
        Write-Error "1. WSL: wsl --install"
        Write-Error "2. Git Bash with expect"
        exit 1
    }
    
    # 임시 파일 정리
    Remove-Item "temp_ssh.exp" -Force -ErrorAction SilentlyContinue
}
...
#!/usr/bin/expect -f
set timeout 30
spawn scp -o StrictHostKeyChecking=no "$LocalFile" $SERVER_USER@$SERVER_IP`:$RemotePath
expect "password:"
send "$SERVER_PASSWORD\r"
expect eof
"@
    
    $expectScript | Out-File -FilePath "temp_scp.exp" -Encoding ASCII
    
    # expect 실행
    if (Get-Command wsl -ErrorAction SilentlyContinue) {
        wsl expect temp_scp.exp
    } elseif (Get-Command bash -ErrorAction SilentlyContinue) {
        bash -c "expect temp_scp.exp"
    } else {
        Write-Error "expect 또는 WSL이 필요합니다."
        exit 1
    }
    
    # 임시 파일 정리
    Remove-Item "temp_scp.exp" -Force -ErrorAction SilentlyContinue
}
...
# 메인 함수
function Main {
    Write-Info "GreenMate Frontend 배포 시작..."
