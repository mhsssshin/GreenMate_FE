# GreenMate Frontend 배포 스크립트 (PowerShell)
# 사용법: .\deploy.ps1
...
function Invoke-SSHCommand {
    ...
    # sshpass 대신 PowerShell의 SSH 사용
    $sshCommand = "echo '$SERVER_PASSWORD' | ssh -o StrictHostKeyChecking=no $SERVER_USER@$SERVER_IP '$Command'"
...
"@
    ...
    # SCP로 파일 업로드 (sshpass 대신 다른 방법 사용)
    $uploadCommand = "echo '$SERVER_PASSWORD' | scp -o StrictHostKeyChecking=no server_deploy.sh $SERVER_USER@$SERVER_IP:/tmp/server_deploy.sh"
    ...
    # 임시 파일 정리
    Remove-Item "server_deploy.sh" -Force
